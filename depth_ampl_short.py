...
then
python3.8 -m pip install TauLidarCommon
...
# make the parent directory (python_modules) available for imports
currentdir = os.path.dirname(os.path.realpath(__file__))
parentdir = os.path.dirname(currentdir)
sys.path.append(parentdir)
print("parentdir: "+parentdir)
 
import mqtt_comms as myComms
...
backgroundFrames = 10
# count of zone crossing between door closures. [0] is down the screen, [1] is up the screen
counts = [0, 0]
mqttDownCount = 0
mqttUpCount = 0
...
lastDoorState = False    # means door is closed
lastCloseTime = 0
waitingDoorState = False
doorCloseDelay = 2	# delay in seconds after door closes for people to clear zone
...
Elements of the blob record are:
    0 position now
    1 blob_id
    2 centre
    3 area
    4 starting position
    5 time entered zone
    6 Image of blob or None
"""
def blobMatching1(thisSet, prevSet):
    global blob_id, upperZoneLine, lowerZoneLine, lostSet
    global mqttUpCount, mqttDownCount
    # compare this set of blobs with
    # the set from the previous frame
    # and try to match the closest
    newSet = thisSet
    lastSet = prevSet
    #print("newSet: "+str(len(newSet))+"    lastSet: "+str(len(lastSet)))
    # get the best overlap between each new blob, and the blobs from the last frame
    ovrList = []
    ovrIndex = []
    for i in range(len(newSet)):
        ...
        ovrList.append(ovr)
        ovrIndex.append(j)
    # go through the list of overlaps for the newSet
    # if any less than 100%, are there any other ones in the 
    # lastSet which have a better overlap than this one
    # if set the index to -1 to indicate that this does not have a good overlap
    for i in range(len(ovrList)):
        if (ovrList[i] < 100):
            j = ovrIndex[i]
            for i2 in range(len(ovrList)):
                if not(i2 == i):
                    if (ovrIndex[i2] == j) and (ovrList[i2] > ovrList[i]):
                        ovrIndex[i] = -1
        
    #print("overlap areas: ", ovrList)
    #print("overlap indexes: ", ovrIndex)
    # which of our newSet has not been matched
    for i in range(len(newSet)):
        if (ovrIndex[i] == -1):
            #print("not matched: ", i)
            ...
            if centre[1] > lowerZoneLine:
                #print("line: 0", newSet[i])
                pass
            if  centre[1] < upperZoneLine:
                #print("line: 1", newSet[i])
                pass
            newSet[i][1] = blob_id
            ...
            # has been matched so update the blob id with the one from the previous frame
            # update the id with the matching blob
            #print("Matched: "+str(i)+"   with: "+str(ovrIndex[i]))
            newSet[i][1] = lastSet[ovrIndex[i]][1]
            newSet[i][4] = lastSet[ovrIndex[i]][4]	# keep the entry point
            newSet[i][5] = lastSet[ovrIndex[i]][5]	# keep the entry time
            if (lastSet[ovrIndex[i]][3] > newSet[i][3]):
                newSet[i][3] = lastSet[ovrIndex[i]][3]	# keep the maximum area of this blob
            pass
    # which of the lastSet could not be matched with the newSet
    for i2 in range(len(lastSet)):
        bFound = False
        for j in range(len(ovrIndex)):
            if (ovrIndex[j] == i2):
                bFound = True
        if not(bFound):
            # this one has not been allocated - so what has happened to it ?
            #print("lost blob: ", lastSet[i2][1], lastSet[i2][0], lastSet[i2][4])
            bLowerD = False
            if (lastSet[i2][0][1] + lastSet[i2][0][3]) > int(0.9*lowerZoneLine):
                bLowerD = True
            bUpperD = False
            if (lastSet[i2][4][1] < int(1.1*upperZoneLine)):
                bUpperD = True        
            if (bLowerD and bUpperD):
                mqttDownCount += 1
                print("DOWN", mqttDownCount)
            bLowerU = False
            #if (lastSet[i2][0][1] + lastSet[i2][0][3]) < int(1.1*upperZoneLine):
            #    bLowerU = True
            if (lastSet[i2][0][1]) < int(1.1*upperZoneLine):
                bLowerU = True
            bUpperU = False
            if (lastSet[i2][4][1] + lastSet[i2][4][3] > int(0.9*lowerZoneLine)):
                bUpperU = True        
            if (bLowerU and bUpperU):
                mqttUpCount += 1
                print("UP", mqttUpCount)
            #print(bUpperD, bLowerD, bUpperU, bLowerU)       
...
def blobMatching(thisSet, prevSet):
    global blob_id, upperZoneLine, lowerZoneLine, lostSet, counts
    # compare this set of blobs with
    # the set from the previous frame
    # and try to match the closest
    newSet = thisSet
...
def removeBackground(newFrame, backgroundArray):
    global backgroundFrames
    if (len(backgroundArray) == 0):
        return newFrame
    else:
        frame = newFrame
        backFr2 = 2*backgroundFrames
        i = 0
        while i < 60:
            ii = int(i/4)
            j = 0
            while j < 160:
                jj = int(j/4)
                if (backgroundArray[ii,jj] > backFr2):
                    for iii in range(4):
                        for jjj in range(4):
                            frame[i+iii, j+jjj] = 0
                j += 4
...
def getDoorOpenStatus():
    global lastDoorState, lastCloseTime, waitingDoorState, doorCloseDelay, button
    if not(button.is_pressed):
        # door is closed
        if lastDoorState and not(waitingDoorState):
            # was open now closed
            waitingDoorState = True
            lastCloseTime = time.time()
        else:
            if (lastDoorState and waitingDoorState):
                # was open and waiting the delay to allow people to move out of the zone
                if (time.time() - lastCloseTime) > doorCloseDelay:
                    lastDoorState = False
    else:
        lastDoorState = True
...
def run(camera, framesDir):
    global blob_id, lower_blue, upper_blue, minBlobSize, lostSet, counts, backgroundFrames
    ...
    while bIsGood:
        ...
        if frame:
            ...
            if (frameCount > 255):
                frameCount = 0
            n = (counts[0]+counts[1])
            if (not(getDoorOpenStatus()) and (n > 0)):
                try:
                    myComms.sendDATAmessage('DATA', (counts[0], counts[1]), 0)
                    #lastEpochSec += nEpoch
                    counts[0] = 0
                    counts[1] = 0
                except:
